...
import { StockCardProps } from 'interfaces/stock';

const StockCard = ({ id, kode, item, location, actual, photo }: StockCardProps) => {
  return (
    <Card
      component={Link}
      to={`/stocks/show/${id}`}
    ...
    >
      ...
      <CardContent sx={{ display: 'flex', flexDirection: 'row', justifyContent: 'space-between', gap:'10px', paddingX: '5px'}}>
        <Stack direction="column" gap={1}>
          <Typography fontSize={16} fontWeight={500} color="#11142d">{item}</Typography>
          <Stack direction="row" gap={0.5} alignItems="flex-start">
            <Place 
              sx={{ fontSize: 18, color: '#11142d', marginTop: 0.5 }}
            />
            <Typography fontSize={14} color="#808191">{kode}</Typography>
        ...
        <Box px={1.5} py={0.5} borderRadius={1} bgcolor="#dadefa" height="fit-content">
          <Typography fontSize={12} fontWeight={600} color="#475be8">${actual}</Typography>
        </Box>
      </CardContent>
    </Card>
  )
